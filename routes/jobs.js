...
function resolveRecord(idOrFilename) {
  ...
  const hit = files.find(
    (f) => f.startsWith(id) && f.toLowerCase().endsWith(".mp4")
  );
  if (hit) {
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (!ok)
      return cb(
        new multer.MulterError("LIMIT_UNEXPECTED_FILE", "Only MP4 is allowed")
      );
    cb(null, true);
  },
});
...
router.post("/subtitle/:id/process", async (req, res, next) => {
  try {
    ...
    if (!rec)
      return res.status(404).json({ ok: false, error: "Upload id not found" });
    ...
    const baseW = W * 0.07; // ~7.0% del ancho
    ...
    const bgmPath = rec.outPath.replace(/\.mp4$/i, "_bgm.mp4");
    await addBackgroundMusic(rec.outPath, musicPath, bgmPath, {
      musicVol: 0.12,
    }); // ~ -18 dB
    rec.outPath = bgmPath;
    ...
    const finalPath = rec.outPath.replace(/\.mp4$/i, "_with_outro.mp4");
    await appendOutroCrossfade(rec.outPath, outroPath, finalPath, {
      durationSec: 0.5,
    });
    rec.outPath = finalPath;
    console.log("Outro appended:", rec.outPath);
    return res.json({
      ok: true,
      id: rec.id,
...
router.get("/subtitle/:id/download", (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${rec.outputName}"`
  );
